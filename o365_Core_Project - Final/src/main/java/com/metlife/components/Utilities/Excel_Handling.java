...
public class Excel_Handling {
    ...
    public static boolean setCellDataWithCondtion(String colName, String rowName, String rowValue, String data) {
        try {
            ...
            for (rowNum = 0; rowNum < row.getLastCellNum() - 1; ++rowNum) {
                System.out.println(rowNum + rowName + row.getCell(rowNum).getStringCellValue());
                s = "";
     ...
     * @Folowing method is to write in existing excel file
     */
    ...
    /**
     * @author Aradhana Jamwal
     * @Folowing method is to write a csv file
     */
     ...
     * @param sheetName     - Name of the worksheet from which data is to be read
     * @param columnIndex   - Column index for which the data has to be fetched/read.
     * @param startRowIndex - Row index starting from where the data has to be fetched/read.
     * @author Aradhana Jamwal
     * @Folowing method is to read the column data starting from the particular row.
     */
    public static ArrayList<String> readExcelColumnData(String filePath, String fileName, String sheetName, int columnIndex, int startRowIndex) throws IOException {
        ...
        System.out.println("Data present in column " + columnIndex + " and starting from row " + startRowIndex + " : " + columnData);
        return columnData;
    }
/*********************************************/
    /**
     * @param csvFilePath - Name of the csv file from which data is to be read and converted to excel
     * @param outputFile  - Name of the excel on which data is to be transferred from csv
     * @author Aradhana Jamwal
     * @Folowing method is to convert the csv data into excel
     */
    public void convertCSVToExcel(String csvFilePath, String outputFile) throws IOException {
        ...
        while (scanner.hasNextLine()) {
            ...
            Row tempRow = outputSheet.getRow(rowNo);
            if (tempRow == null)
                tempRow = outputSheet.createRow(rowNo);
